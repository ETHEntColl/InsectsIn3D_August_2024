...
class MetashapeHelper():
    ...
    def closeHoles(self):
        ...
        # Update current task name and number labels in window
        self.window.update_task_info("Close Holes", 3, self.task_amount) 

        # Save the start time of the task
        start_time = time.time()

        # Log the task start
        self.logger.log_task_start("Close Holes")

        # Execute task
        self.document.chunk.model.closeHoles(
            level=100,
        )
    ...
    def exportModel(self):
        ...
        # Update current task name and number labels in window
        self.window.update_task_info('Export Model', 4, self.task_amount)
