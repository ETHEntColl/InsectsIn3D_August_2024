...
class DatasetHelper():
    ...
    def create_dataset_object(self, dataset_path: str) -> Dataset:
        ...
        image_folder_path   = os.path.join(dataset_path, settings.get('image_folder_path'))
        model_folder_path   = os.path.join(dataset_path, settings.get('model_folder_path'))
        cam_pos_file_path   = os.path.join(dataset_path, settings.get('cam_pos_file_path'))
        scan_info_file_path = os.path.join(dataset_path, settings.get('scan_info_file_path'))
    ...
    def get_image_paths(self, image_folder_path: str) -> List[str]:
        ...
        if os.path.exists(image_folder_path):
            for entry in os.scandir(image_folder_path):
                if entry.is_file() and entry.name.lower().endswith(tuple(settings.get('image_extensions'))):
